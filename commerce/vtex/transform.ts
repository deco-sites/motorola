...
const getSkuVariations = (
  product: LegacyProductVTEX | ProductVTEX,
  url: URL,
) => {
  ...
  // deno-lint-ignore no-explicit-any
  items.map((item: any) => {
    variations[item.itemId] = {};
    if (item?.variations?.length > 0) {
      // deno-lint-ignore no-explicit-any
      item.variations.map((variation: any) => {
        //adding self
        if (!variations[item.itemId][variation.name]) {
          variations[item.itemId][variation.name] = [];
        }
        variations[item.itemId][variation.name] = [{
          value: variation.values[0],
          url: getProductURL(url, product, item.itemId),
        }];
        //adding others
        // deno-lint-ignore no-explicit-any
        items.map((item2: any) => {
          const item2Var = item2.variations;
          if (item2.itemId !== item.itemId) {
            if (item2Var?.length > 0) {
              // deno-lint-ignore no-explicit-any
              item2Var.map((item2variation: any) => {
                const sameVar = item2variation.name == variation.name;
                if (sameVar) {
                  if (variation.values[0] !== item2variation.values[0]) {
                    if (!variations[item.itemId][variation.name]) {
                      variations[item.itemId][variation.name] = [];
                    }
                    variations[item.itemId][variation.name].push({
                      value: item2variation.values[0],
                      url: getProductURL(url, product, item2.itemId),
                    });
                  }
                }
              });
...
export const toProduct = <P extends LegacyProductVTEX | ProductVTEX>(
  ...
  const skuVariations = getSkuVariations(product, url);
